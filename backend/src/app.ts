...
const OAUTH_FAST_LANE_TIMEOUT_MS = 15_000;
const WEBHOOK_REQUEST_TIMEOUT_MS = 20_000;
const BILLING_REQUEST_TIMEOUT_MS = 24_000;
...
const resolveRequestTimeoutMs = (path: string) => {
  if (
    OAUTH_FAST_LANE_TIMEOUT_PATH_PREFIXES.some((prefix) => path.startsWith(prefix))
  ) {
...
app.use((req, res, next) => {
  ...
  res.end = ((...args: unknown[]) => {
    if (isAnalyticsDashboard) {
      logAnalyticsDashboardLifecycle("res.end invoked", {
        requestId: req.requestId || null,
        elapsedMs: getAnalyticsDashboardLifecycleElapsedMs({ res }),
        headersSent: res.headersSent,
        writableEnded: res.writableEnded,
    ...
    const response = (originalEnd as (...endArgs: unknown[]) => typeof res)(...args);
    if (isAnalyticsDashboard) {
      logAnalyticsDashboardLifecycle("res.end returned", {
        requestId: req.requestId || null,
        elapsedMs: getAnalyticsDashboardLifecycleElapsedMs({ res }),
        headersSent: res.headersSent,
        writableEnded: res.writableEnded,
...
app.post("/v1/messages", optionalApiKeyAuth, asyncHandler(async (req: any, res) => {
  try {
    ...
    const jobs = await enqueueAIBatch(messages, {
      source: "api",
      idempotencyKey:
