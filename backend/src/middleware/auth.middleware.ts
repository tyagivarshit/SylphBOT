...
const isInstantCheckoutRoute = (req: Request) => {
  const route = String(req.originalUrl || req.path || req.url || "")
    .trim()
    .toLowerCase();
  const path = String(req.path || "").trim().toLowerCase();
  ...
  return (
    route.startsWith("/api/billing/checkout/instant") ||
...
export const protect = async (
  ...
  try {
    ...
    if (isInstantCheckoutRoute(req) && accessToken) {
      ...
      const maybeContext =
        decoded?.id && typeof decoded.tokenVersion === "number"
