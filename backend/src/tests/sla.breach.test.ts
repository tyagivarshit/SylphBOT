...
export const slaBreachTests: TestCase[] = [
  {
    name: "sla monitor breaches critical unresolved work to owner route",
    run: async () => {
      ...
      try {
        (prisma.humanWorkQueue as any).findMany = async () => [
          {
            ...
            priority: "HIGH",
            state: "IN_PROGRESS",
            slaDeadline: new Date("2026-04-27T09:55:00.000Z"),
            interaction: {
              id: "interaction_1",
        ...
        assert.deepEqual(events, ["sla.breached"]);
        assert.equal(interactionUpdates[0].priorityLevel, "CRITICAL");
        assert.equal(interactionUpdates[0].routeDecision, "OWNER");
        assert.equal(queueUpdates[0].queueType, "OWNER_REVIEW");
        assert.equal(queueUpdates[0].state, "ESCALATED");
      } finally {
