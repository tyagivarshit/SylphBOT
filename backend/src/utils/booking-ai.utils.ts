...
*/
export const parseTimeFromText = (text: string): {
  hours: number;
  minutes: number;
} | null => {
...
export const findClosestSlot = (
  ...
  let minDiff = Math.abs(
    requested.getTime() - closest.getTime()
  );

  for (const slot of availableSlots) {
    const diff = Math.abs(
      requested.getTime() - slot.getTime()
    );
...
*/
export const formatSlotsList = (slots: Date[]): string => {
  return slots
