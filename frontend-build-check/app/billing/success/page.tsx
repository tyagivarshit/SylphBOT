...
const shouldStopAfterConfirmFailure = (result: CheckoutConfirmResponse | null) =>
  Boolean(
    result &&
      result.state === "FAILED" &&
      !result.shouldPoll &&
      TERMINAL_CONFIRM_CODES.has(String(result.code || "").trim().toUpperCase())
  );
    ...
    const activate = async () => {
      ...
      if (sessionId) {
        ...
        if (!cancelled && shouldStopAfterConfirmFailure(confirmResult)) {
          const immediateBilling = await fetchBilling();

          if (!cancelled && isActivated(immediateBilling, expectedPlan)) {
            setResolvedPlan(getPlanLabel(immediateBilling));
            setMessage(getSuccessMessage(immediateBilling));
            setLoading(false);
            return;
          }
